...
-(void) writeVideoFrameAtTime:(CMTime)time addImage:(CGImageRef )newImage
{
    if (![videoWriterInput isReadyForMoreMediaData]) {
		...
		@synchronized (self) {
            ...
            size_t bytesPerRow = CGImageGetBytesPerRow(newImage);
            
            CFDataRef  dataFromImageDataProvider = CGDataProviderCopyData(CGImageGetDataProvider(newImage));
            GLubyte  *imageData = (GLubyte *)CFDataGetBytePtr(dataFromImageDataProvider);
            
           int status = CVPixelBufferCreateWithBytes(kCFAllocatorDefault,width,height,kCVPixelFormatType_32ARGB,imageData,bytesPerRow,NULL,NULL,(__bridge CFDictionaryRef)options,&pixelBuffer);
